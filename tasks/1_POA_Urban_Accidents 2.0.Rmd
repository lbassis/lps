...
df %>% group_by (NOITE_DIA) %>% summarize(TOTAL = n()) %>% 
  ggplot(aes(x=NOITE_DIA, y=TOTAL)) +
  geom_col(width = 0.5) +
  theme_bw() +
  labs(x = "PERIOD", y = "AMOUNT OF ACCIDENTS");
```
...
df %>% summarize(AUTO = sum(AUTO),
               TAXI = sum(TAXI),
               LOTACAO = sum(LOTACAO),
               ONIBUS_URB = sum(ONIBUS_URB),
               ONIBUS_INT = sum(ONIBUS_INT),
               CAMINHAO = sum(CAMINHAO),
               MOTO = sum(MOTO),
               CARROCA = sum(CARROCA),
               BICICLETA = sum(BICICLETA),
               OUTRO = sum(OUTRO)) %>% t() %>% as.data.frame() %>% rename(AMOUNT_OF_ACCIDENTS = V1) %>%
  ggplot(aes(x=rownames(.), y=AMOUNT_OF_ACCIDENTS, fill=rownames(.))) + 
  geom_col() +
  theme(axis.title.x = element_blank(), axis.text.x = element_blank()) +
  labs(fill = "VEHICLES")

#%>% t() %>% as.data.frame %>% rename(AMOUNT = V1) # %>% arrange(desc(Amount))
...
df %>% group_by(TIPO_ACID) %>% summarize(DEATHS = sum(MORTES),
  ...
  ggplot(aes(x=TIPO_ACID, y=DEATHS, fill=TIPO_ACID)) +
  geom_col() +
  theme_classic() +
  theme(axis.title.x = element_blank(), axis.text.x = element_blank())
```
